...
#define TXD_PIN GPIO_NUM_44  // 随便选一组空闲引脚
        ...
        if (strcmp(type->valuestring, "tts") == 0) {
            ...
            if (emotion != NULL) {
                Schedule([this, display, emotion_str = std::string(emotion->valuestring)]() {
                    ...
                    uint8_t emotion_code = 0;
                    static const char* EMOTION_NAMES[] = {
                        "angry", "confident", "confused", "cool", "crying", "delicious", "embarrassed", "funny",
                        "happy", "kissy", "laughing", "loving", "neutral", "relaxed", "sad",
                        "shocked", "silly", "sleepy", "surprised", "thinking", "winking"
                    };
                    for (uint8_t i = 0; i < sizeof(EMOTION_NAMES) / sizeof(EMOTION_NAMES[0]); ++i) {
                        if (emotion_str == EMOTION_NAMES[i]) {
                            emotion_code = i;
                            break;
                        }
                    }
                    uart_write_bytes(UART_NUM_1, &emotion_code, 1);
                });
            }
